...
# network. Rather than hard-code either, read the first IPv4 nameserver the runtime itself wrote
# into this container's resolv.conf and render the `resolver` directive that nginx.conf includes.
#
...
resolver=$(awk '$1 == "nameserver" && $2 ~ /^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$/ { print $2; exit }' "$resolv_conf")
if [ -z "$resolver" ]; then
  echo "[nginx-entrypoint] ERROR: no IPv4 nameserver in ${resolv_conf}; cannot enable upstream re-resolution." >&2
  exit 1
fi
printf 'resolver %s ipv6=off valid=10s;\n' "$resolver" > "$resolver_conf"
echo "[nginx-entrypoint] upstream resolver: ${resolver} (from ${resolv_conf})"
