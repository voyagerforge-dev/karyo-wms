// Requires @pact-foundation/pact >= 17.1.3. Below that floor the mock server commits a
// request's match result AFTER writing the response, so a concurrent run can read
// pre-commit state and fail a request the server already served. Do not downgrade.
import { PactV3, MatchersV3 } from '@pact-foundation/pact';
import { describe, it, expect } from 'vitest';

const { like, eachLike, integer, string, decimal } = MatchersV3;

const provider = new PactV3({
  consumer: 'karyo-dashboard',
  provider: 'product-service',
  dir: './pacts',
});

describe('Product API Contract', () => {
  it('returns paginated products', async () => {
    await provider
      .given('products exist')
      .uponReceiving('a request for paginated products')
      .withRequest({
        method: 'GET',
        path: '/api/v1/products',
        query: { page: '0', size: '20' },
      })
      .willRespondWith({
        status: 200,
        headers: { 'Content-Type': 'application/json' },
        body: {
          content: eachLike({
            id: integer(1),
            number: string('SKU-001'),
            name: string('Test Product'),
            state: integer(0),
            scale: integer(0),
            lotMandatory: like(false),
            bestBeforeMandatory: like(false),
            serialNoRecordType: string('NO_RECORD'),
            itemUnit: like({
              id: integer(1),
              name: string('Piece'),
              unitType: string('PIECE'),
            }),
            numbers: eachLike({
              id: integer(1),
              number: string('EAN-001'),
            }),
            packagingUnits: eachLike({
              id: integer(1),
              name: string('Box'),
              amount: decimal(12.0),
              itemUnitName: string('Piece'),
              packingLevel: integer(1),
            }),
            created: string('2026-01-01T00:00:00Z'),
            modified: string('2026-01-01T00:00:00Z'),
          }),
          page: {
            number: integer(0),
            size: integer(20),
            totalElements: integer(1),
            totalPages: integer(1),
          },
        },
      });

    await provider.executeTest(async (mockServer) => {
      const response = await fetch(
        `${mockServer.url}/api/v1/products?page=0&size=20`
      );
      expect(response.status).toBe(200);
      const data = await response.json();
      expect(data.content).toHaveLength(1);
      expect(data.content[0].number).toBeDefined();
      expect(data.page).toBeDefined();
    });
  });

  it('creates a product', async () => {
    await provider
      .given('item units exist')
      .uponReceiving('a request to create a product')
      .withRequest({
        method: 'POST',
        path: '/api/v1/products',
        headers: { 'Content-Type': 'application/json' },
        body: {
          number: 'SKU-NEW',
          name: 'New Product',
          itemUnitId: 1,
        },
      })
      .willRespondWith({
        status: 201,
        headers: { 'Content-Type': 'application/json' },
        body: {
          id: integer(2),
          number: string('SKU-NEW'),
          name: string('New Product'),
          state: integer(0),
        },
      });

    await provider.executeTest(async (mockServer) => {
      const response = await fetch(`${mockServer.url}/api/v1/products`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          number: 'SKU-NEW',
          name: 'New Product',
          itemUnitId: 1,
        }),
      });
      expect(response.status).toBe(201);
      const data = await response.json();
      expect(data.number).toBe('SKU-NEW');
    });
  });
});
