...
class ShipmentRestTest {
    ...
    fun `open packing then pack via REST finishes the shipment and order at PACKED`() {
        seedPackStaging()
        ...
        given().contentType(ContentType.JSON).body("""{"weight":2.5,"type":"CARTON"}""")
            ...
            .body("state", `is`(650))
            .body("shippingUnits.size()", `is`(1))

        given().`when`().get("/api/v1/shipments/$shipmentId").then().statusCode(200).body("state", `is`(650))
    ...
    @OidcSecurity(claims = [Claim(key = "client_id", value = "1"), Claim(key = "tenant_code", value = "ACME")])
    fun `open packing is forbidden without fulfillment-write`() {
        given().contentType(ContentType.JSON).body("""{"deliveryOrderId":1}""")
            .`when`().post("/api/v1/shipments").then().statusCode(403)
    }
}
