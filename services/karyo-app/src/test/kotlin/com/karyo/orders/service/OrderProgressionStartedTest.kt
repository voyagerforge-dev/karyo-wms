...
/**
 * Integration test (REAL beans) for the [OrderProgressionPort.markStarted] write seam: seeds and
 * releases an order, then advances it to STARTED via the SPI and verifies the new state through REST.
 */
@QuarkusTest
class OrderProgressionStartedTest {

    @Inject
    lateinit var progression: OrderProgressionPort

    // ── REST seeding helpers (lifted from OrderProgressionPortTest) ──────────
