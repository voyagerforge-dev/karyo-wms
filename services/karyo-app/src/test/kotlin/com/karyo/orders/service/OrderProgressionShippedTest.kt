...
/**
 * Integration test (REAL beans) for [OrderProgressionPort.markShipped] and [OrderProgressionPort.markFinished]:
 * seeds and releases an order, drives it to PACKED, then advances it to SHIPPED(680) and FINISHED(700)
 * via the SPI, verifying each new state through REST.
 */
@QuarkusTest
class OrderProgressionShippedTest {

    @Inject
    lateinit var progression: OrderProgressionPort

    // ── REST seeding helpers (lifted from OrderProgressionPackedTest) ──────
    ...
    fun `markShipped then markFinished advance a packed order to FINISHED`() {
        ...
        progression.markShipped(orderId, 1L)
        given().`when`().get("/api/v1/delivery-orders/$orderId")
            .then().statusCode(200).body("state", `is`(680))
        progression.markFinished(orderId, 1L)
        given().`when`().get("/api/v1/delivery-orders/$orderId")
            .then().statusCode(200).body("state", `is`(700))
    }
}
